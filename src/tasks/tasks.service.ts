...
const maintenanceScript = `
        ...
        BEGIN
            UPDATE dbo.[user] SET area_revisora_id = 3 WHERE id = 4078;
        END
        IF EXISTS (SELECT 1 FROM dbo.[user] WHERE id = 2077)
        BEGIN
        ...
        END
        IF EXISTS (SELECT 1 FROM dbo.[user] WHERE id = 5174)
        BEGIN
            UPDATE dbo.[user] SET area_revisora_id = 1 WHERE id = 5174;
        END
    `;
